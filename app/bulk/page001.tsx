                            ...
                            {groupedHistory.map(pGroup => {
                                ...
                                return (
                                    <div key={pKey} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                                        ...
                                        {/* 날짜별 상세 */}
                                        {isExpanded && (
                                            <div className="border-t border-slate-50 divide-y divide-slate-100">
                                                {pGroup.dailyGroups.map(dGroup => (
                                                    <div key={dGroup.date} className="px-4 py-3 bg-slate-50/40">
                                                        {/* 날짜 카드 - 2026년 2월 23일만 두꺼운 초록색 테두리 */}
                                                        <div className={`${dGroup.date === '2026-02-23' ? 'border-4 border-green-500 rounded-lg p-2 mb-2' : 'border-2 border-green-400 rounded-xl p-3 bg-slate-50/40'}`}>
                                                            <p className="text-[11px] font-black text-slate-500">
                                                                {new Date(dGroup.date).toLocaleDateString('ko-KR', { month: 'long', day: 'numeric', weekday: 'short' })}
                                                            </p>
                                                        </div>
                                                        {/* 거래 목록 - 날짜가 2026-02-23이면 테두리 안의 내용 패딩 조정 */}
                                                        <div className={`space-y-1.5 ${dGroup.date === '2026-02-23' ? 'px-1' : 'pl-3'}`}>
                                                            {dGroup.transactions.map(tx => (
                                                                <div key={tx.txKey}
                                                                    onClick={() => handleEditModal(tx.records, pGroup.companyName)}
                                                                    className="flex items-center justify-between bg-white rounded-xl px-3 py-2 border border-slate-100 cursor-pointer hover:border-indigo-200 hover:bg-indigo-50/30 transition-all group">
                                                                    {/* 거래 요약 */}
